...
#define x real()
#define y imag()
const T PI=3.141592653589793238462643383279502884L,EPS=1e-6;
bool eq(T a,T b){return abs(a-b)<EPS;}

int sgn(T a){return (a>0)-(a<0);}
T sq(T a){return a*a;}
namespace std{bool operator<(P l,P r){return make_pair(l.x,l.y)<make_pair(r.x,r.y);}}
...
P rot(P a,T theta){return a*polar((T)1,theta);}
P rot(P a,T theta,P p){return rot(a-p,theta)+p;}
double angle(P a,P b,P c){return abs(remainder(arg(a-b)-arg(c-b),PI));}
bool onSeg(P p, P a, P b) {return cross(a,b,p) == 0 && dot(p-a,p-b) <= 0;}
P intersect(P a,P b,P c,P d){
    T x1=cross(c-a,b-a),x2=cross(d-a,b-a);
    return (x1*d-x2*c)/(x1-x2);
}
T lineDist(P p,P a,P b){
    return abs(cross(a-p,b-p))/abs(a-b);
}
