...
const ll N=200010,V=27,M=pow2(61)-1,B=randll(256,M-1),B0=randll(256,M-1);
...
{
    return {(hashnum[r]-mul(hashnum[l-1],p[r-l+1])+M)%M,(hashnum0[r]-mul(hashnum0[l-1],p0[r-l+1])+M)%M};//(hashnum[r]-hashnum[l-1]*p[r-l+1]%M+M)%M;
}
...
int main(){
    ...
    fori(1,m)
        hashnum[i]=(mul(hashnum[i-1],B)+sb[i]+1)%M;
    fori(1,m)
        hashnum0[i]=(mul(hashnum0[i-1],B0)+sb[i]+1)%M;
    fori(1,n)
    {
        ned[sa[i]-'a']++;
        cnt[sb[i]-'a']++;
