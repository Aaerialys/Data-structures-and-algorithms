...
struct wavelet{
    ...
    int query(int lo,int hi,int k,int ind=1,int l=0,int r=N-1){ //kth min number in lo+1 to hi
        if(l==r) return l;
...
int main(){
    ...
    for(auto& i:num) i=cord[i];
    st.build(num);
    fori(0,q){
        cin>>a>>b>>c;
        cout<<poi[st.query(a-1,b,c)]<<"\n";
    }
    return 0;
}
/**
